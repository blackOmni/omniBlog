...
const getAllBlogs = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, { blogs }, 'Blogs found'));
  } catch (e) {
...
const addBlog = async (req, res) => {
  ...
  try {
    ...
    if (!existingUser) {
      return res.status(400).json(new ApiError(400, 'Unauthorized user'));
    }
...
const getByUserId = async (req, res) => {
  ...
  try {
    ...
    if (!userBlogs) {
      return res
        .status(404)
        .json(new ApiError(404, 'No blog found for this user'));
    }
    return res
      .status(200)
      .json(
